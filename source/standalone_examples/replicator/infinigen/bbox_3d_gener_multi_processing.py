...
def simulation_process(proc_id: int, task_queue: Queue, status_dict: Dict):
    ...
    while True:
        try:
            ...
            if not task_queue.empty():
                ...
                print(f"[SimProcess {proc_id}] 开始任务 {task_id}")
                run_sdg(config, task)

    if status_dict[task_id] != "CANCELLED":
                        status_dict[task_id] = "FINISHED"
                        print(f"[SimProcess {proc_id}] 完成任务 {task_id}")
